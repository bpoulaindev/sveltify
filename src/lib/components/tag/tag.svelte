<script lang="ts">
    export type TagColor = 'green' | 'indigo' | 'red' | 'zinc' | 'yellow' | 'pink' | 'purple' | 'blue'
    export let color: TagColor | undefined
    ...
    } as { [key in TagColor]: string }
    if (!color) {
        const colorOptions: TagColor[] = ['green', 'indigo', 'red', 'zinc', 'yellow', 'pink', 'purple', 'blue'];
