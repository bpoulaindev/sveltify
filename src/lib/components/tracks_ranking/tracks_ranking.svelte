...
<div
  class="flex w-full max-h-[calc(100dvh-348px)] xs:max-h-[calc(100dvh-368px)] sm:max-h-[calc(100dvh-388px)] lg:max-h-[calc(100dvh-228px)] {classes}">
  {#if !$tracksRankingStore?.total || $tracksRankingStore?.total === 0}
    <div
      class="flex flex-col animate-pulse-3 items-center h-auto w-full overflow-y-scroll rounded-2xl px-2 py-0.5 sm:py-1 bg-white dark:bg-zinc-800 shadow-xl">
      {#each Array(5) as _}
  ...
  {:else}
    <div
      class="flex flex-col items-center px-1 sm:px-2 py-0.5 sm:py-1 w-full h-auto overflow-scroll rounded-2xl bg-white dark:bg-zinc-800 shadow-xl">
